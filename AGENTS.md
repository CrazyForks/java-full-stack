...
3. 编码前读该文件的里程碑状态表与当前卡所属段落（历史里程碑按需查阅），只实现前置已完成且状态为「待开始」的第一张卡片。
...
原则：可部署配置从配置源读取；跨模块机器契约（错误码、权限码、存储键等）按职责具名集中；局部声明（路由、endpoint、表单校验、展示文案、单测数据等）就近保留字面量，字符串重复本身不构成抽常量理由。改动相关值后用 `rg` 扫描旧字面量并说明保留理由。

细则只在两处定义：后端见 [boot-server/AGENTS.md](boot-server/AGENTS.md) 第 4 节，前端见 [前端开发规范](docs/10-实战产品蓝图/前端开发规范.md) 第 10 节。
...
并行原则：实施类 Subagent 仅在任务范围明确且互不依赖时并行；探索与规则读取默认串行、按需最小化，不做启动时全量扫描。有依赖的任务必须串行（实现 → 测试 → 验收）。

主会话已读取的规则约束，须在任务说明中摘要转述给 Subagent；Subagent 只读与任务直接相关的规范节选，不重复通读规则链。
