...
| 分词 / 分析器 | 将原文或查询分析成可检索的 token（词元） | 查询 token 要能与索引 token 匹配；中文可评估 `ik` 等分析器 |
...
- **你需要明确的点**：ES 用倒排索引和分词解决检索问题，但要付出同步与运维成本；Spring `@Scheduled` 适合简单任务，XXL-Job 等平台解决分片、追踪和告警。先按需求选，不按组件名套答案。
...
> **生活版——同一业务事实的两种视图**：前端路由配置与页面导航会围绕同一组页面建立不同表示；类似地，MySQL 可保存交易事实，ES 保存为检索场景裁剪、分词和反规范化后的派生文档。二者字段不必完全相同，职责也不同：源库负责事务事实，ES 负责搜索与聚合，靠同步和对账维持可重建关系。
>
...
>
> **换成 ES**：写入文档时分析器先把文本切碎成**词条（Term）**，登记「每个词出现在哪些文档」；查询时按词直接反查这张「词 → 文档 ID」表，再求交 / 求并组合多词条件。它避免了逐行扫描原文，但实际成本仍取决于词的文档频率、posting list 长度、分片数、过滤条件和排序，不能简化成固定的 O(词条数)。
...
- **分析器（Analyzer）**会将文本经过字符过滤、分词与 token 过滤，产出倒排索引使用的 token。中文通常需要词典/规则驱动的分析器；IK 是常见的第三方插件，不是 Elasticsearch 内置能力。
  - `ik_max_word`：倾向把文本切出更多候选 token，常被用于索引侧以提升召回；token 更多意味着索引体积和歧义处理也要评估。
  - `ik_smart`：倾向更粗粒度切分，常被用于查询侧以减少查询 token；它不是“天然更精准”的保证。
  - **真正原则是 token 可匹配**：查询分析器产生的 token 必须能与索引侧已有 token 形成预期命中。索引、查询使用同一分析器是安全的起点；像 `ik_max_word` 建索引、`ik_smart` 查询也是常见组合，但必须用代表性词、同义词、短语和负例通过 `_analyze` / 集成测试验证，不能把两个名称的分工当铁律。[Elastic 官方：分析器与 `_analyze`](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis.html)
...
- **日志事实源与搜索索引要分开**：应用输出的结构化日志/审计事件及其采集管道是“发生过什么”的原始证据；ES 索引是为检索、聚合和可视化建立的派生视图。索引模板变化、重建、滚动删除或同步失败都不应改写事实含义；需要长留存或合规审计时，原始日志应有独立的持久化、访问控制与保留策略，不能只依赖一个可删除的 ES 索引。业务数据同理：MySQL 等业务库是事务事实源，ES 是可重建的搜索索引。
...
- [ ] 能区分业务事实源、原始日志/审计证据与 ES 搜索索引，说明为什么索引重建或 ILM 删除不能成为“事实被删除”的唯一依据
...
- 标题用 `text` 配分析器，状态、品牌、排序和聚合字段用 `keyword` 或 multi-field；价格用数值类型。索引和查询分析器只要 token 匹配即可，不要求名称相同；用 `_analyze` 与代表性搜索词验证召回/误召回。
- 需要业务事件语义时优先评估 MQ 异步；已有 MySQL 且需捕获所有变更时可评估 Canal，前提是能处理 binlog、位点、DDL 和对账。
- MySQL 等业务库与原始日志/审计事件各自承担事实记录；ES 是按 mapping 建出的检索视图。索引重建、滚动和 ILM 删除应有独立的原始日志保留、备份及权限策略配合。
- 公共搜索可接受近实时延迟；发布者页面可读主库或在写入后使用明确的可见性策略。无论哪种，都不能把 refresh 当成跨系统事务。日志与业务事实源都应独立于 ES 搜索索引保存，ES 可重建、可滚动删除。
...
- ES 在写入时就把文本切成词元、建好「词 → 文档 ID」的倒排表，查询时可直接处理相关 posting list 并求交/求并，而不是逐行扫描原文；实际耗时仍取决于词频、候选集、分片和排序。
- 这也是为什么 ES 叫全文检索引擎——**数据结构就是为搜索设计的**。

**工程实践**：
- 前提是**token 匹配符合搜索意图**（中文可评估 IK 等分析器；索引与查询分析器相同是安全起点，不同组合必须以 `_analyze` 和搜索测试验证）。
