...
- **你需要明确的点**：工具环境是「写第一行代码之前就要就位」的——JDK + IDEA + Maven 三件套到位，一套命令自检通过，后续学习才不会被打断；遇到环境问题先回查本清单。

### 最简可运行示例（照抄能跑）

装完三件套后，先在本教程默认的 Windows PowerShell 中验收：
...
> 期望 `java` 与 `javac` 都显示 25，`JAVA_HOME` 指向 JDK 根目录，`where.exe java` 的第一条结果与预期发行版一致。环境变量修改后要重新打开终端。剩下的是用 IDEA 建一个 Maven 工程跑通 `mvn test`（见正文第 5 节）。

### 环境搭建三件事（先做这些）

1. **装 JDK 25**：推荐 Temurin 或 Corretto 发行版，装完用 `java -version` 确认是 25；长期支持期限以所选发行版的支持策略为准。
2. **装 IntelliJ IDEA**：社区版即可起步，用它新建一个空 Maven 工程跑通 Hello World。
3. **配 Maven**：能 `mvn compile` / `mvn test` 通过，依赖能下载下来。

### 常用约定 / 版本速查

- **版本基线**：本计划以 **Java 25** 为基线。LTS（长期支持）是 Oracle、Temurin 等发行版的支持承诺，不是 Java SE 规范给语言版本统一贴的属性；安装时同时查看发行版的维护周期。
...
- **支持版本理解**：17、21、25 常被主要发行商作为长期支持版本提供，但具体年限和许可由发行商决定。本学习计划使用 Java 25，不把“LTS”误写成 Java SE 语言规范属性。

```powershell
# Windows PowerShell：安装后验证
java -version
javac -version
$env:JAVA_HOME
where.exe java
...
```xml
<!-- 父 pom：BOM（Bill of Materials，版本清单）集中约束依赖版本；它不等于锁定完整依赖图的 lockfile -->
...
- 前端工程化经验可以迁移到“集中管理模块和版本”的目标上，但机制不同：`<modules>` 类似 workspace 聚合，BOM / `dependencyManagement` 只约束声明到的版本，**不等同于记录完整解析结果的 lockfile**。
...
```powershell
java -version                    # ① 运行时版本为 25
javac -version                   # ② 编译器版本为 25
$env:JAVA_HOME                   # ③ 指向 JDK 根目录，不要带 \bin
where.exe java                   # ④ 第一条命中路径与预期 JDK 一致
mvn -v                           # ⑤ 同时核对 Maven 实际使用的 Java home
mvn test                         # ⑥ 在新建的 Maven 工程里跑通
```

> 如果使用 `cmd.exe`，查看变量写 `echo %JAVA_HOME%`；如果使用 Git Bash，才写 `echo $JAVA_HOME`。三种终端语法不能混用。`Path` 中应配置 `%JAVA_HOME%\bin`，而 `JAVA_HOME` 自身应指向 JDK 根目录。

## 坑点提醒

- `JAVA_HOME` 指向错误目录、旧 JDK 或误带 `\bin` → Maven 可能找不到工具或使用错误版本；多版本共存时同时检查 `$env:JAVA_HOME`、`where.exe java` 和 `mvn -v`。
...
1. npm 用 lockfile 记录完整解析结果；Maven 项目通常怎样控制依赖版本和构建输入？为什么把 `-Dmaven.repo.local` 指向固定目录只能复用缓存，不能保证版本确定性？（提示：显式版本、父 POM / BOM、插件版本、内部仓库与校验、固定 JDK 和 CI 环境）
...
- 与 npm 的关键差异：Maven 通常用显式版本、父 POM / BOM、插件版本、受控远程仓库和固定构建环境控制输入；`-Dmaven.repo.local=...` 只改变本地仓库（下载缓存）位置，不会替你锁定依赖图。依赖冲突仍可能导致 `NoSuchMethodError`，因此要检查实际解析树并在 CI 重建验证。

### Q2：JDK、JRE、JVM 三者是什么关系？为什么现在很少单独提 JRE 了？

**答**：**标准结论**：按职责理解，JVM 负责执行字节码，Java 运行环境还需要标准类库等运行时组件，JDK 再提供 `javac`、`jlink`、调试器等开发工具。`JDK ⊃ 运行时组件 ⊃ JVM` 是便于入门的概念关系，不代表现代 JDK 磁盘中仍有一个独立的 `jre/` 子目录。

**工程层**（为什么 JRE 提得少了）：JDK 9、10 仍有独立 JRE 下载；从 JDK 11 起，Oracle 不再提供独立 JRE / Server JRE 下载，JDK 11 安装镜像也不再包含旧式 `jre/` 镜像。需要精简运行时可用 `jlink` 构建；其他发行商的打包方式以其支持策略为准。**完整版**见[起步基础知识](05-起步基础知识.md)第 1 节与 Q5。
