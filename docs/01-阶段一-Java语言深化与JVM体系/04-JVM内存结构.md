...
- **你需要明确的点**：**对象在堆、方法调用在栈、类定义在元空间**。排查内存问题时先问「是堆满了、栈溢出，还是元空间满了」——这三类 OOM 的处理完全不同。
...
| 概念 / 参数 | 干什么 | 最易踩的坑 |
|-|-|-|
| `-Xms` / `-Xmx` | 堆的**初始 / 最大**大小 | 需为元空间、线程栈与直接内存留余量；超过容器或主机可用内存可能导致启动失败或被系统终止 |
...
| 元空间 | 类的元信息（类结构 / 方法字节码 / **运行时常量池**）——JDK 8 起在本地内存，替代旧「永久代」 | `OutOfMemoryError: Metaspace` |
...
**JDK 7 与 JDK 8 的迁移不要混为一谈**：JDK 7 已将 `String.intern()` 对应的 StringTable 中字符串放到 Java 堆；JDK 8 才移除永久代（PermGen），把类元信息及每个类的**运行时常量池**放入本地内存的元空间（Metaspace）。两者虽然都常被称为「常量池」，却不是同一件事：运行时常量池是每个类的符号/字面量元数据；StringTable 是用于复用 intern 字符串的查找表，字符串对象仍在堆中。元空间采用不同的容量模型，但 `MaxMetaspaceSize`、本地内存不足或类加载器泄漏仍可能导致 `OutOfMemoryError: Metaspace`，不是「从此不会 OOM」。
...
**场景核对**：接口偶发 `StackOverflowError`，优先检查递归是否有出口及调用深度，而不是先增大堆；若报 `Java heap space`，再通过 heap dump 查哪类对象或集合持续增长。两者都叫“内存问题”，定位入口不同。
...
- ③ **内容变化与版本**：JDK 7 已将 `String.intern()` 对应的字符串移到堆；JDK 8 再将类元信息和**运行时常量池**从永久代迁入元空间。不要把 StringTable 与运行时常量池混成一个概念。

**工程层**（为什么换 + 面试注意）：
- 为什么换：永久代的容量模型与动态类加载（框架代理、热部署、Groovy 等）容易冲突；元空间改用本地内存并可按需扩展，降低了该类配置摩擦，但仍须监控类加载器泄漏、元空间上限和进程总内存。
