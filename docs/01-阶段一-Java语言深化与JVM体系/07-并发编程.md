...
| 关键词 | 最低解释与典型用途 | 使用边界 |
|-|-|-|
| 线程 / 线程池 | 线程是并发执行单元；线程池复用平台线程，如 `ThreadPoolExecutor` | 池要有界队列、命名工厂和拒绝策略，不能把参数照搬到所有服务 |
| CAS / `AtomicInteger` | 比较再交换的原子操作，适合单变量计数 | `i++` 不原子；多变量不变量仍需锁或协调机制 |
| AQS / `ReentrantLock` | JUC 锁工具的公共底座；可重入锁用于复合临界区 | 解锁放进 `finally`；优先理解语义，再选锁 |
| `CompletableFuture` / `Future` | 表示未来结果并组合异步任务 | IO 任务应传入专用执行器，取结果设置超时 |
| 虚拟线程 | JVM 的轻量线程，适合大量阻塞 IO 任务 | CPU 密集任务仍受核心数限制；不需要为了复用而池化 |

### 本讲在解决什么问题

- **问题**：服务器要同时处理成千上万的请求，每个请求可能要去等数据库/HTTP（IO 等待）。如果每个请求占一个 OS 线程，几百个请求就能把系统拖垮；如果不用异步，等待时间又白白浪费。
- **你需要明确的点**：**IO 密集**（等 DB/HTTP/文件）适合虚拟线程（阻塞不占 OS 线程）；**CPU 密集**（大量计算）才需要固定大小线程池（核数是硬上限）。线程池要显式参数，别用快捷工厂。
...
- 掌握层次：会上面三个工具类 + 能讲清「state 的 CAS 修改 + 获取失败的线程入队挂起」即可，AQS 源码精读排在[学习时间规划](../学习时间规划.md)的贯穿任务里。
...
- **线程池**：预先创建一组线程反复复用，避免「每任务建线程」的创建销毁开销——概念上等价 Node 的 Worker 池或 Go 的 goroutine 调度。Java 平台线程通常 1:1 对应一条操作系统线程，栈大小受 `-Xss` 和平台配置影响，因此常用**有界**线程池限制并发与资源占用；这不适用于“池化虚拟线程”。
...
public class PoolFactory {
    public static ThreadPoolExecutor ioPool() {
        return new ThreadPoolExecutor(
            ...
            new ArrayBlockingQueue<>(200),          // 有界队列：满后才会进入拒绝策略，避免无限积压
...
| 队列满后 | 扩线程到 max → 仍满 → **调用拒绝策略**；`Abort` 抛异常，`CallerRuns` 由提交线程执行 | 容量近似无限；未关闭的执行器通常不会因“队列满”而拒绝 |
| 结局 | 有限的排队内存；若拒绝策略不静默丢弃，过载可观测、可处理 | 提交速率持续高于处理速率时，任务与其负载不断积压，先拉高延迟，最终可能耗尽堆内存而 OOM |
...
**生活版（把排号凳讲透）**：餐厅门口的排号凳是**有限的**——坐满了才招临时工（扩到 max）、连凳子都没了才按既定规则处理新客（例如拒绝，或让点单员自己做菜）。要是排号凳**无限长**（无界队列）：短期内不会因为“没座位”触发拒绝，队伍却会在到店速度长期大于出餐速度时越排越长，先表现为排队延迟，最后可能把登记簿撑爆（OOM）。

**换成 Java**：队列是压力的「蓄水池」——有界才有「满」的概念，「满」才能依次触发扩容与拒绝这些保护动作；无界队列会把过载藏进延迟和内存。选择明确容量的 `ArrayBlockingQueue` 或 `LinkedBlockingQueue`；容量要结合可接受排队时延、任务大小、下游承载与压测结果设定，而不是只照抄一个数字。
...
- **拒绝策略选型**：`CallerRuns`（反压减速——调用者线程自己执行任务，把压力反弹给上游；不能用于事件循环、持锁路径等不应被阻塞的提交者）/ `Abort`（抛异常，默认，调用方要处理）/ `Discard`（静默丢，生产慎用）。执行器已关闭时，`CallerRuns` 也会丢弃任务，不能把它当成可靠投递保证。
- **工程红线**：避免不经评估就用 `Executors.newFixedThreadPool`（内部是无界 `LinkedBlockingQueue`，持续过载会积压到 OOM）与 `newCachedThreadPool`（最大线程 `Integer.MAX_VALUE`，可能耗尽线程/本地内存）；需要隔离、容量和拒绝语义时显式构造 `ThreadPoolExecutor(...)`。
...
- **CompletableFuture**：Java 版 Promise——一个「将来才有值」的容器，可链式组合、可聚合、可兜底。与 `async/await` 的差异：没有语法糖，靠链式调用；不传执行器会走公共 `ForkJoinPool`，所以 **IO 阻塞或需要资源隔离的任务应显式指定受控执行器**，短小 CPU 计算才评估是否适合公共池。
...
public class AggregateDemo {
    public static void main(String[] args) throws Exception {
        // 此处是 IO 聚合：显式定义容量与拒绝语义，避免把阻塞任务放进公共池。
        try (var pool = new ThreadPoolExecutor(
...
public final class NativeGateway {
    // 教学片段：省略 JNI 库的编译/加载；真实项目只能调用已审计的 native 库。
    // 若 nativeCall 内部发生长时间阻塞，调用它的虚拟线程会在调用期间 pin 住载体线程。
    static native void nativeCall();

    static void callFromVirtualThread() {
        nativeCall();
    }
}
```

- **Pinning（钉住）**：虚拟线程在阻塞时本可从载体（platform/carrier）线程卸载；被 pin 时不能卸载，载体直到调用返回才可服务其他虚拟线程。JDK 24 的 JEP 491 已消除 `synchronized` 监视器导致的 pinning，因此 **Java 25 不应再为规避 pinning 而机械地把 `synchronized` 改成 `ReentrantLock`**。
- **Java 25 仍要看的边界**：虚拟线程执行 JNI native 方法或 FFM（Foreign Function & Memory API）外部函数期间仍可能 pin；只有该 native/foreign 调用本身很慢或阻塞，才会把问题放大为载体线程被长期占用。它是吞吐/隔离风险，不等同于业务逻辑错误。
- **工程处理**：先用 JFR/线程转储确认真实阻塞点；为外部库调用设置超时、并发上限与隔离策略。若库无法避免长阻塞，放进受控的专用执行器只是隔离影响，不会让该调用“自动 unpin”；同时评估下游连接与 native 资源上限。
- **不要池化虚拟线程**：它是廉价的一次性资源，用平台线程的「复用」心智去管理它是反模式。
  - **生活版**：服务员（平台线程）是稀缺资源所以要复用——客人等菜时他不闲着，去招呼别的桌；而订单（虚拟线程）不是资源，随用随撕，用完扔掉即可，没有「回收再用」的必要。
  - **换成 Java**：虚拟线程不一一对应 OS 线程，栈按需由 JVM 管理，创建成本足够低，不应靠线程池限制它的数量；需要限制的是数据库连接、外部 API 等下游资源。为虚拟线程另建池只会额外引入排队与生命周期管理。
...
> 排查要点：
> - **现象**：`RejectedExecutionException`——任务提交被拒。常见原因是有界队列已满且线程已到上限，或执行器已关闭。
> - **原因**：任务量超过处理能力（下游慢/并发高）或任务长期阻塞（如 `Future.get()` 无超时、DB 连接池耗尽）会造成上述“满”。**无界队列**通常不会产生“队列满”的拒绝；它会先累积延迟和内存压力，最终可能 OOM。
...
- 外部依赖上的 `Future.get()` 没有超时/取消策略：下游卡死时可能无限等待；按业务 SLA 设计超时、降级或取消，而非无条件阻塞。

## 本节自检

- [ ] 能回答：虚拟线程适合 IO 密集还是 CPU 密集？Java 24 后 `synchronized` 的 pinning 边界如何变化？Java 25 为什么仍要审计长时间的 JNI/FFM 调用？
...
- [ ] 场景：订单服务有大量数据库等待和少量报表计算。能说明两类任务分别为什么适合虚拟线程或受限大小的平台线程池，并写出核对要点：前者关注阻塞与下游承载，后者关注 CPU 核数与任务拆分。

> 核对要点：数据库等待是 IO 阻塞，虚拟线程可减少平台线程被占用；报表计算受 CPU 核数约束，应限制并行度。两类任务都要考虑下游连接池或服务的承载上限。

## 本节配套思考题

1. 200 个并发请求打到一个 `core=8 / max=8 / queue=200 / 拒绝策略=Abort` 的线程池，第 209 个请求会发生什么？换成 `CallerRuns` 呢（提示：想想提交线程被占用后的连锁反应；执行器关闭时又如何）？
...
- **原理层**：
  - 平台线程通常 1:1 映射到 OS 线程，栈大小取决于 `-Xss` 与平台；虚拟线程（Project Loom，JDK 21 正式版）是 JVM 调度的轻量线程，许多 JDK 管理的阻塞点会让它卸载载体线程去运行其他虚拟线程——因此「一请求一线程」的同步写法可服务大量 IO 等待并发，但仍受下游资源限制。
  - 为什么替代不了 CPU 密集：CPU 密集任务不需要等待，虚拟线程的挂载/卸载调度反而变成纯开销——此时该用并行流 / 固定大小线程池（核数是硬上限）。
- **工程层**：
  - 虚拟线程不要池化（用完即弃）。
  - Java 25 中，`synchronized` 监视器不再是 pinning 来源；仍需审计 JNI/FFM 外部调用是否长时间阻塞载体线程，并做超时、限流和隔离。
