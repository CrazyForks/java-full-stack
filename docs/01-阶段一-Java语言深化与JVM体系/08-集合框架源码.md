...
| 关键词 | 最低解释与典型用途 | 使用边界 |
|-|-|-|
| `List` / `ArrayList` | 有序可重复；动态数组适合按下标读和尾插 | 频繁头部插删要整体搬移 |
| `Set` / `HashSet` | 去重集合，依赖 `equals` / `hashCode` | 相等对象必须有相同哈希值 |
| `Map` / `HashMap` | 键值查找，平均情况下查找快 | 扩容有开销，且不能并发写 |
| `ConcurrentHashMap` | 面向共享 Map 的并发容器 | 安全不等于所有复合操作自动原子 |
| `CopyOnWriteArrayList` | 写时复制，适合读多写极少 | 写入会复制数组，频繁写代价高 |
| 扩容 / fail-fast | 容量不足时扩容；遍历时检测结构修改 | fail-fast 是尽力的错误检测，不提供并发保护 |

### 本讲在解决什么问题

- **问题**：业务里大量用到列表、去重、按键查找。不同场景选不同集合（数组/链表/哈希表/树），选错就慢或错。还要理解为什么并发修改会抛异常。
- **你需要明确的点**：**查得快用 HashMap、要顺序用 ArrayList、要线程安全共享用 ConcurrentHashMap**。`HashMap` 之所以查得快，是「先算 hash 定位桶，再在桶里找」；迭代时被修改会抛 `ConcurrentModificationException`——这**不是并发保护，是错误检测**。
...
- [ ] 场景：商品标签既要去重，又要在多个请求线程中更新。能说明为什么普通 `HashSet` 不能直接共享，并核对方案是否同时满足去重语义、并发安全和复合操作的原子性。

> 核对要点：`HashSet` 不提供并发写安全；可按操作选择并发集合或外部同步。若“检查后再写入”等步骤必须整体原子，不能只依赖单个线程安全方法。
