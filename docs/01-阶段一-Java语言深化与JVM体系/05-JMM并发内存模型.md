...
- **你需要明确的点**：`i++` 之所以丢更新，是因为它其实是「读-改-写」三步，不是原子操作；`volatile` 能保证可见性，但不保证原子性。看到并发问题，先想「有没有 happens-before 保证」。
...
- 铁律：**`try { ... } finally { threadLocal.remove(); }`**——尤其平台线程池复用场景。虚拟线程通常是“每任务一个、任务结束即终止”，不应把它当作会复用同一线程的线程池；但长生命周期虚拟线程或在一个线程内承载多个逻辑任务时，仍应及时清理上下文。
...
- 治理一：铁律 `try { ... } finally { threadLocal.remove(); }`——尤其平台线程池复用场景；任务型虚拟线程通常不复用，但长生命周期或多逻辑任务共用的虚拟线程仍需清理。
