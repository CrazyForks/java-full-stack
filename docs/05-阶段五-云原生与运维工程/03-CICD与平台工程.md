...
| 环境晋级（Promotion） | 同一制品依次进入不同环境 | 不为生产环境重新构建镜像 |

### 本讲在解决什么问题

- **问题**：改完代码怎么从「本地能跑」到「线上稳定」？靠手动部署容易出错。CI/CD 把「构建→测试→部署」变成自动、可重复、可回滚的流水线。
- **你需要明确的点**：CI（Continuous Integration，持续集成）是「每次提交自动跑测试+构建」，CD 是「自动发布到环境」。注意 CD 有两个层次：**持续交付（Continuous Delivery）** = 随时可发布、按按钮上线；**持续部署（Continuous Deployment）** = 合并即自动上线。**门禁（Quality Gate）是灵魂**——测试不过、覆盖率不够就不让合并不让发布。

### 速览形态示意（需结合仓库配置）

> 示例展示 GitHub Actions 的 CI 骨架；能否运行取决于仓库中的 Maven、JaCoCo、Dockerfile、权限和 Runner 环境。`app:1.0` 也是示意标签，生产流水线还需生成不可变版本或 digest，并补制品推送、签名和部署门禁。
...
- [ ] **场景判断**：staging 验证通过后，团队为生产重新执行一次 Docker 构建；能说明这破坏了什么保证，并给出正确的环境晋级方式

> 自检答案要点：重新构建会生成未经 staging 验证的新制品，即使源码相同，基础镜像、依赖仓库和时间也可能改变结果。应让同一镜像 digest 从 dev 晋级到 staging、prod，环境差异通过受控配置注入，并保留审批、审计和回滚记录。
