...
**答**：
- **标准结论**：现代 JDK 默认支持识别容器资源限制，但仍需核对所用 JDK、cgroup 版本与运行参数；识别失败或堆外余量不足都可能触发 cgroup OOM Kill。可用 `-XX:MaxRAMPercentage` 或显式 `-Xmx` 控制堆，并为元空间、线程栈和直接内存留余量。
- **底层原理**：
...
- **工程实践**：
  - **版本差异**：较新的 JDK 8 更新版和 JDK 10+ 已提供容器感知，但支持范围与默认行为随发行版、更新号和 cgroup 版本变化；用 `-XshowSettings:system` 等方式在目标镜像实测。
