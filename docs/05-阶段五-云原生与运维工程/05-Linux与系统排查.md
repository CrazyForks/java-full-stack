...
| `top -Hp` | 查看某进程内各线程的 CPU | 记录 TID 后与线程栈对应 |
| `ss` / `lsof` | 查看网络连接与文件占用 | 排查连接数、监听端口和文件句柄 |

### 本讲在解决什么问题

- **问题**：线上 CPU 飙高、内存 OOM、连接池耗尽——你只能看日志猜？这一讲教你用 OS 工具 + Java 诊断工具，**从「现象」定位到「代码行」**。
- **你要带走的一句话**：排查主线是「**top 看进程 → top -Hp 看线程 → printf 线程 id 转 16 进制 → jstack 看线程栈 → 缩小到热点调用路径**」。是否能直接定位源码行还取决于调试信息、JIT 内联和采样时机，必要时再用 Arthas 或 async-profiler 交叉验证。

### 速览命令模板（替换占位符后执行）

> `<PID>`、`<TID>` 和 `<16进制nid>` 都是占位符；命令依赖 Linux 的 procps、JDK 诊断工具与当前用户权限。容器内还要确认 PID 命名空间和工具是否存在，生产抓栈前评估输出量与执行影响。
...
- [ ] **场景判断**：容器被标记为 OOMKilled，但应用日志里没有 `OutOfMemoryError`；能给出区分 Java 堆 OOM、堆外增长和 cgroup 上限触发的证据链

> 自检答案要点：先查看容器终止原因、内存 limit 与工作集曲线，再对照 JVM 堆、非堆、直接内存和线程数指标；Java 堆 OOM 通常有异常或 heap dump，cgroup 杀进程可能来不及留下 JVM 异常。结合 `jcmd`/NMT、GC 指标及宿主机事件判断，不能仅凭“容器重启”认定堆泄漏。
