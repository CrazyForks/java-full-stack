...
| `.subscribe()` | 启动订阅的终点操作 | Web 框架会订阅返回值，业务层通常不手动订阅 |
...
> `Mono` 和 `Flux` 来自 Reactor；代码省略构建依赖（例如引入 Reactor Core）和错误处理，仅演示发布者、操作符与订阅的关系。WebFlux Controller 返回 `Mono`/`Flux` 时由框架订阅，不能照搬此处手动 `subscribe()`。
...
| 并发能力 | 降低阻塞等待占用平台线程的成本；容量仍取决于连接、数据库与限流 | 少量事件循环处理非阻塞 I/O；容量仍取决于下游与背压策略 |
| 纪律成本 | 较低：可保留同步写法，但仍要控制并发与下游容量 | 较高：需优先使用非阻塞依赖，隔离少量不可避免的阻塞调用 |
...
# Servlet 容器可用虚拟线程处理请求；实际容量仍须压测数据库、连接池和下游服务
```
...
- **在 Reactor Netty 的 WebFlux 链路里直接调用同步 JDBC**：会占住事件循环，拖慢同一事件循环上的请求。应优先改用非阻塞驱动；暂时不能改时，把阻塞源通过 `subscribeOn(Schedulers.boundedElastic())` 隔离，并限制并发。`boundedElastic` 只是隔离线程池，不会替你解决背压、连接池或慢任务堆积。
...
- [ ] **场景判断**：一个 Servlet MVC 项目新增普通 CRUD，但没有持续流、背压或网关管道需求时，能选择 MVC（可按需启用虚拟线程）并说明不引入 WebFlux 的理由。

> 场景题核对：关键不是并发数字，而是是否需要端到端非阻塞流和 Reactive Streams 背压；若底层仍是 JDBC 等阻塞依赖，改 WebFlux 不能自动消除阻塞成本。
...
**答**：**标准答案**：许多普通 CRUD 系统不需要——虚拟线程（Java 21+，可通过 `spring.threads.virtual.enabled=true` 启用）降低「一请求一线程」处理阻塞 I/O 的线程成本，仍需按数据库、连接池和下游容量压测。WebFlux 更适合以下三类场景：
