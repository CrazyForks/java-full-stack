...
- **你需要明确的点**：你写的每个 `@Service` / `@Autowired`，背后都是容器在替你完成「创建 + 连接」——所以**别在构造器里用依赖、别用 `this` 调事务方法**，因为容器给你的永远是「代理 + 已注入好的对象」。
...
**循环依赖**：A 构造需要 B，B 构造又需要 A。先分清两个层次：Spring Framework 在**显式允许**循环依赖时，能借三级缓存处理一部分单例的 setter / 字段注入环；Spring Boot 默认 `spring.main.allow-circular-references=false`，因此这类环也会在启动期被拒绝。下面的图解释的是前者的容器内部机制，不是推荐在 Boot 项目中保留依赖环：
...
- **构造器注入的循环依赖不能由三级缓存处理**——默认配置下会启动失败；`@Lazy` 可以通过注入延迟代理把取用推后，但这不是消除依赖环。长期方案仍是拆出共同职责或调整依赖方向。
...
- [ ] 能说明 Boot 默认为什么拒绝循环依赖；在显式允许时，能画图讲清三级缓存如何处理部分 setter / 字段注入环，以及为什么它不能处理构造器注入环
...
**答**：**标准答案**：先说边界：Spring Framework 在允许循环依赖时，可用三级缓存处理部分单例 setter / 字段注入环；Spring Boot 默认拒绝循环依赖，工程上应拆环。其内部机制是——
