...
- **你需要明确的点**：同步调用要配**超时 + 重试 + 幂等**三件套；异步通信用 **Outbox** 把「业务状态 + 待投递记录」在本地事务中原子持久化，再由 relay 重试投递、告警并让消费端幂等处理。它通常形成**至少一次**投递，不承诺绝对不丢。**通信的可靠性纪律比通信方式更重要。**
...
| Outbox | 业务状态与**待投递记录**放进同一个本地事务 | relay、重试/告警或消费幂等缺失，仍可能遗漏或重复副作用 |
| 事件驱动 | 发事实、谁关心谁订阅（如 OrderPaid） | 事件可能重复到达，消费端仍要幂等 |
| Dubbo 3 | 国内存量 RPC，协议 Triple（兼容 gRPC） | 深度治理特性，学习阶段不用碰 |
| 服务网格 | 通信治理下沉到 Sidecar 代理，业务代码通常少改 | 仍要处理注入、流量与观测；中小团队容易背运维债 |
...
flowchart TD
    ...
    C["客户端（只建一条连接）"] --> M
    M --> S["服务端：并行处理，分别响应"]
    O["HTTP/1.1 单连接且无流水线时<br/>前一个响应会阻塞后续响应<br/>客户端常以连接池缓解"] -. 对比 .-> M
```

**卖点三：IDL 契约先行**——`.proto` 是机器可读契约，工具生成各语言强类型客户端与服务端**桩代码**（stub：按契约生成接口壳，业务逻辑留空）。它能减少方法和字段签名漂移，但语义、兼容策略和部署版本仍要靠契约测试与发布治理：
...
|-|-|-|-|
| 协议 | 声明式 HTTP；常用 JSON，HTTP 版本取决于底层客户端与配置 | Triple（基于 HTTP/2，可与 gRPC 生态互通）及其他 Dubbo 协议 | HTTP/2 + Protobuf |
| 定位 | Spring Cloud 存量生态常见、声明式上手快 | 国内企业存量较多，治理能力丰富 | 跨语言、IDL 契约与流式 RPC |
| 服务发现 | 可复用注册中心（如 Nacos） | 支持应用级服务发现，减少接口级注册元数据 | 通常结合 K8s Service、xDS 或自建解析 |
| 选型依据 | 简单性 | 性能 + 治理能力 | 多语言 + IDL 契约 |

> 注：**Nacos** = 注册与配置平台；**Triple** = Dubbo 3 的 HTTP/2 RPC 协议；**应用级服务发现** = 以应用粒度注册，再在元数据中表达接口能力，实际元数据降幅取决于接口规模；**K8s Service** = K8s 内置的服务寻址与负载均衡抽象；**IDL** = 先定义跨语言契约，再生成各语言代码。Spring 官方已将 OpenFeign 定位为 feature-complete，并建议新项目评估 Spring HTTP Service Clients；存量 Spring Cloud Alibaba 项目仍可按兼容矩阵使用 Feign。[Spring Cloud OpenFeign 官方说明](https://docs.spring.io/spring-cloud-openfeign/reference/index.html)
...
**事件驱动（Event-Driven）**：服务间不点名调用，而是“发布事实、订阅方自行响应”——积分 / 物流 / 通知可各自监听 `OrderPaid`。新增订阅方通常不必修改发送方业务流程，但事件契约、权限、容量和兼容性仍要治理。异步化可以把瞬时高峰暂存在队列中，让消费端按能力处理；前提是 Broker 容量、积压告警和降级策略足以承受洪峰。
...
**问题**：「本地事务成功」与「事件发出」无法用一次本地事务原子化；裸写 `save(); publish();`——publish 前崩溃 = 没有待投递证据，publish 后崩溃 = 状态不一致。标准答案是 **Outbox（事务性发件箱，Transactional Outbox）模式**：把“直接发消息”降级成“同事务写待投递记录”，原子性交给数据库，再由 relay 投递。
...
```java
// 消费端：新增订阅方通常不改发送方流程，但必须遵守事件契约并独立治理失败
@Component
...
> **为什么消费端必须幂等——「至少一次投递」（at-least-once）**：relay 在“MQ 已收到、发送方尚未来得及标记 sent”时会重试，所以同一事件可能到达多次。Outbox 将业务状态与待投递记录原子持久化；还要靠 relay 的重试、死信/告警、恢复演练与消费幂等把风险收敛，不能承诺所有故障下绝对不丢。工程标准解是「**至少一次 + 消费端幂等**」——与阶段三第 5 讲（消息队列）的 MQ 幂等是同款纪律。
...
- **Istio / Envoy 的 Sidecar 模式**：每个 Pod 旁挂一个代理，接管进出流量——mTLS（双向 TLS 认证：客户端与服务端互验证书，防中间人。**内网 ≠ 免认证**——服务之间也要验对方身份，mTLS 是它平台化的答案）、重试 / 熔断 / 超时、流量镜像（复制一份影子流量打新版本）、全链路追踪都可从「各应用分别实现」下沉为「平台统一实现」。业务代码通常无需逐处接入 SDK，但仍要完成 Sidecar 注入、流量劫持、端口 / 协议兼容和观测验证，不能理解成部署后天然零改造。
- **适用边界**：收益出现在「多语言 / 大组织统一治理」；代价是每个 Pod 多一个代理（资源 + 延迟 + 运维复杂度）。**中小团队优先 Spring Cloud Gateway + K8s 原生能力**——别为了「先进」背 Sidecar 的运维债。

> **前端对照**：Service Worker 可以在页面与网络之间拦截请求，帮助理解“把通信能力放到代理层”。边界是：页面仍要完成注册、作用域和缓存策略适配；服务网格也要做注入、协议识别、流量规则与观测验证，并非业务完全零感知。

> ⏸️ **短期可以不学**：服务网格（Service Mesh）的深入——Istio / Envoy 的 Sidecar 配置、VirtualService / DestinationRule 规则、mTLS 与流量镜像的落地。本讲只需理解「把通信能力下沉到代理、业务代码通常少改」的边界；Sidecar 注入、流量劫持和观测验证仍是部署责任。**何时回来学**：公司真有多语言、大组织统一治理的诉求要上服务网格时。**面试最低要求**：一句话说出 Sidecar 模式是什么、收益（治理能力平台化、业务代码通常少改）与代价（资源 + 运维复杂度）。
...
**工程实践**：拿不准就先同步——异步化引入的对账成本、事件丢失排查成本别低估；已上异步的系统，用 Outbox 原子持久化「业务状态 + 待投递记录」，再以 relay 重试/告警、对账和消费幂等收敛投递风险。

### Q3：Outbox 模式解决什么问题？和 RocketMQ 事务消息怎么选？

**标准结论**：Outbox（事务性发件箱）解决“本地事务成功却没有待投递证据”的问题——业务表和 outbox 事件表**同库同事务**写入，原子性只覆盖“业务状态 + 待投递记录”，不覆盖网络上的 MQ 投递。
...
- 中小企业、已有 MQ 的场景可优先评估 Outbox；注意消费端仍要做幂等（至少一次投递）——Outbox 保证的是“业务状态与待投递记录不分家”，不是“所有故障下投递绝对不丢”或“消费只一次”。
...
- **IDL 先行**：`.proto` 作为机器可读契约生成多语言客户端与服务端桩，能在编译期减少字段和方法签名漂移；语义变更、兼容性规则和部署版本不一致仍需契约测试与发布治理。

**底层原理（与 Feign/REST 的区别）**：REST 常以资源语义和 JSON 表达，浏览器与通用 HTTP 工具支持好；gRPC 面向方法、使用 Protobuf 并支持流式调用，适合多语言强契约或对带宽 / 编解码敏感的内部链路。gRPC 不保证所有负载都更快，仍要按消息大小、连接、代理和端到端延迟压测；代价包括代码生成、契约兼容和基础设施支持。

**工程实践**：存量 Spring Cloud 项目可继续使用 Feign；新项目同时评估 Spring HTTP Service Clients。需要 Protobuf 强契约、双向流或跨语言调用时再考虑 gRPC，并用真实负载验证收益，而不是按“内部调用默认某框架”决定。
